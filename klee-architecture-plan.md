...
- 完全本地推理，数据私密
- Apple Silicon 原生优化，Apple 官方战略框架（M5 Neural Accelerators 专属加速）
...
| 性能 | 单流 decode 与 llama.cpp 持平；内存效率更优（~50%）；M5 Neural Accelerators 专属加速 |
| 零配置 | 用户无需安装任何东西 |
...
|---|---|---|
| Decode 速度（单流） | 与 llama.cpp 持平（8B+ 模型受内存带宽限制） | 基准线 |
| 内存效率 | **更优**（约 50%，可跑更大模型） | 较高 |
| Swift 集成 | **官方 SPM，原生 API** | 需 C 桥接（unsafeFlags 问题） |
| 未来硬件加速 | **M5 Neural Accelerators 专属加速**（TTFT 4x） | 无法利用 |
| 模型格式 | safetensors (MLX) | GGUF（生态最广） |
| 维护方 | **Apple ML Research** | 社区 |
| 跨平台 | 仅 Apple Silicon | 全平台 |

**实测验证（M4 Pro 24GB，Qwen3 8B 4-bit）**：Klee (MLX) 44.3 tok/s ≈ Ollama (llama.cpp) 44.1 tok/s。单流 decode 受内存带宽限制，引擎差异不大。MLX 的真正优势在 Swift 原生集成、内存效率和未来硬件加速。

**结论**：Klee 选 MLX 不是因为"更快"，而是 Apple 官方战略框架 + Swift 原生 + 零外部依赖 + 未来硬件加速。
...
**实测基准（2026-03-19，M4 Pro 24GB）：**

**Qwen3 8B（标准 Transformer）— 三引擎持平：**

| 指标 | Klee (MLX Swift) | Ollama (llama.cpp) | oMLX (MLX Python) |
|------|-----------------|-------------------|-------------------|
| **Decode** | **44.3 tok/s** | **44.1 tok/s** | **44.6 tok/s** |
| **Prompt** | 235 tok/s | 11.3 tok/s | 406 tok/s |

**Qwen3.5 9B（GatedDeltaNet 混合架构）— Swift 较慢：**

| 指标 | Klee (MLX Swift) | oMLX (MLX Python) | 差距 |
|------|-----------------|-------------------|------|
| **Decode** | 19.9 tok/s | 44.6 tok/s | **2.24x** |
| **Prompt** | 51 tok/s | 406 tok/s (缓存后) | — |

**关键发现**：标准 Transformer 模型（Qwen3）上 Klee 已达硬件极限（44.3 tok/s），与 Ollama/oMLX 持平。性能差距**仅存在于 Qwen3.5 的 GatedDeltaNet 混合注意力架构**——这是 mlx-swift-lm 对新架构的适配尚未优化，不是框架层面的通用问题。

**当前策略**：不 fork，跟踪 mlx-swift-lm 上游对 GatedDeltaNet 的优化，优先推进模块开发。
