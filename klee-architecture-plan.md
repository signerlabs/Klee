...
#### 方案 B：VLM Adapter 模式（P0，主要优化路径）

> **oMLX 实践证实：不需要双模型通道。** 单模型 + VLMModelAdapter 路由即可实现纯文本跳过视觉编码器，比维护两套加载路径更简单可靠。

参考 oMLX 的 `VLMModelAdapter` 方案，将 VLM 的 `language_model` 暴露为标准 LLM 接口，三种前向路径：

```swift
/// VLM adapter: routes requests through the appropriate path
/// - Text-only: calls language_model directly, skips vision encoder
/// - With images: runs vision encoder -> language_model
/// - Batch VLM: left-padded embeddings + token alignment
class VLMAdapter {
    private let vlmContainer: ModelContainer

    /// Text-only: bypass vision encoder entirely
    func generateText(input: LMInput) async -> AsyncStream<GenerationChunk> { ... }

    /// Multimodal: run vision encoder + language model
    func generateWithImages(input: LMInput, images: [UserInput.Image]) async -> AsyncStream<GenerationChunk> { ... }
}
```

**实现路径**：需 fork mlx-swift-lm 或提交 PR，在 Qwen35.swift 中将 `visionModel` 改为 optional / lazy init。已确认 MLXLLM 中有 `Qwen35Model`（包装 `languageModel: Qwen35TextModel`），其 `sanitize()` 已经会过滤 `vision_tower` / `model.visual` 权重——说明上游也预留了纯文本路径的空间。

**预期收益**：单一模型文件同时服务纯文本和多模态请求，纯文本时零视觉开销。

**~~方案 A：双模型通道~~**（已废弃）

oMLX 证实维护 LLM/VLM 双通道加载没有必要——VLM Adapter 用单个模型实例路由更简洁。双通道方案的额外内存开销和切换延迟是不必要的代价。
...
| **B: VLM Adapter（参考 oMLX）** | 2-3 周 | 纯文本跳过视觉编码器 | **P0** |
| **C: SSD KV 缓存** | 4-6 周 | Agent 场景 TTFT 降低 5-20x | P1 |
| **oMLX 后端集成** | 3-5 天 | 高级用户立即可用 | P2 |

**核心原则：VLM Adapter 是下一步最重要的优化（方案 B），然后是 SSD KV 缓存（方案 C）。**
...
Phase C: VLM Adapter 模式（2-3 周，参考 oMLX）
├── Fork 或 PR mlx-swift-lm：visionModel 改为 lazy init
├── 实现 VLMAdapter（单模型实例，三种前向路径）
│   ├── 纯文本：直接调用 language_model，跳过视觉编码器
│   ├── 有图片：运行视觉编码器 → language_model
│   └── 批量 VLM：embeddings 对齐 + 左填充
├── LLMService 集成 VLMAdapter
├── 纯文本场景对比基准测试
└── 预期：纯文本速度接近 Qwen3-8B 水平
...
Phase F: SSD KV 缓存（4-6 周）
├── 实现 KVCacheBlock + 链式哈希前缀匹配
├── 处理 Qwen3.5 混合缓存（MambaCache + KVCacheSimple）
├── 实现 SSD 持久化层（safetensors 格式）
├── 三级缓存：GPU → RAM (LRU) → SSD
