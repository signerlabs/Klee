...
### 架构决策：Swift MCP SDK + 内嵌 Node.js LTS（方案 B）

**调研结论（2026-03）：**

| 方案 | 评估结果 |
|---|---|
| **方案 A：包 OpenClaw 进 Klee** | ❌ node_modules ~1.4GB；OpenClaw 不能直接调用进程内 MLX，需额外起 HTTP 服务进程（多一层 IPC），破坏零配置架构；过度工程化 |
| Bun 替代 Node.js | ❌ Playwright MCP 已确认无响应（Issue #25861），native addon 兼容率仅 34% |
| **方案 B：Swift MCP SDK + 内嵌 Node.js LTS** | ✅ MLX 推理与 MCP Client 同在主进程，零 IPC 开销；Node.js 仅用于运行社区 MCP Server 子进程；100% npm 生态兼容；bundle ~150MB |

**为什么不包 OpenClaw：** AutoClaw / QClaw 包 OpenClaw 是因为它们的核心产品就是 OpenClaw Skills 生态。Klee 的核心是 MLX 原生推理，MCP 是扩展层，只需要标准 MCP 协议栈即可，无需依赖 OpenClaw 私有 Skills 范式。

**App Store 限制：** 内嵌 Node.js 需要 `com.apple.security.cs.allow-jit` entitlement，App Store 不允许，因此 Phase 2 必须走 Developer ID 分发。这也是 Claude Desktop、LM Studio、Cursor 的共同选择。
...
| 产品 | 框架 | 推理引擎 | MCP Agent | 零配置 | 开源 |
|------|------|---------|-----------|--------|------|
| **Klee** | **SwiftUI 原生** | **MLX Swift** | **Phase 2** | **是** | 是 |
| LM Studio | Electron | llama.cpp + MLX (内嵌) | ✅ v0.3.17+ | 是 | 部分开源 |
| Jan.ai | Electron | Cortex.cpp (llama.cpp) | ✅ 内置 | 是 | 是 (AGPLv3) |
| Swama | SwiftUI 菜单栏 | **MLX Swift** | ❌ | 是 | 是 (MIT) |
| GPT4All | Qt/QML | llama.cpp | ❌ | 是 | 是 (MIT) |
| Msty Studio | Electron | MLX (alpha) | ✅ | 是 | 否 |
| Cherry Studio | Electron | 桥接 Ollama | ✅ | 是 | 是 |
| Ollama App | Go + Web | llama.cpp (Metal) | ❌ | 是 | 是 |
| AutoClaw（智谱） | Node.js Bundle | llama.cpp (via Ollama) | ✅ (OpenClaw) | 是 | 否 |
| QClaw（腾讯） | Node.js Bundle | llama.cpp (via Ollama) | ✅ (OpenClaw) | 是 | 否 |

**Klee 的独特定位：全球唯一同时具备 MLX Swift 原生推理 + SwiftUI 原生 UI + MCP Agent 的本地 AI 客户端。**

### 关键竞品详解

#### LM Studio（最强竞品）
- v0.3.4 起内嵌 mlx-engine（MIT 开源），v0.3.17 起支持 MCP Host
- **已是 MLX + MCP 的成熟组合**，是目前功能最全面的竞品
- 致命弱点：Electron 应用，macOS 原生体验差，内存开销 ~300MB+
- 融资 $19.3M，有能力后续推出原生 macOS 版本——**Klee 的最大威胁**

#### Swama（技术路线最接近）
- 纯 Swift + MLX Swift 推理，比 Ollama 快 1.5-2x
- **架构本质**：和 Ollama 完全相同的模式——CLI 工具 + 后台 HTTP 服务进程
  - `swama serve` 启动后在 `0.0.0.0:28100` 监听 OpenAI 兼容 API
  - macOS 版提供菜单栏 App，启动即自动后台运行服务，等同 Ollama 的菜单栏行为
  - 没有聊天界面，只是 API 服务端
- 安装方式：`brew install swama` 或 GitHub Releases DMG（~30MB）
- 支持 Whisper 转录、文本 Embedding，内建 SwiftNIO HTTP Server
- **缺少**：Chat UI、MCP、对话历史——只是基础设施层工具，不面向终端用户

#### AutoClaw（智谱）/ QClaw（腾讯）
- **打包方式**：把 Node.js LTS + OpenClaw Gateway 依赖打成安装包，一键完成原本需要手动执行的命令行操作
- **不是** Electron 应用，不是 fork OpenClaw，就是"安装器 + 预配置"
- **本地模型支持**：继承 OpenClaw 原生 Ollama 集成（底层 llama.cpp），非 MLX，不是默认路径
- 默认推理走云端 API（AutoClaw → GLM-5，QClaw → Kimi-2.5），"本地部署"指 Gateway 进程跑在本地
- **对 Klee 的参考价值**：验证了 Node.js Bundle 打包方案（Phase 2 规划的内嵌 Node.js 方案）在量产环境可行

### 差异化优势

1. **性能**：MLX 在 Apple Silicon 上比所有竞品的 llama.cpp 快 21%-87%
2. **体积**：Phase 1 仅 ~70MB，竞品普遍 300MB+（Swama 30MB 但仅是 CLI）
3. **内存**：无 Electron/Qt 运行时开销，更多内存留给模型
4. **原生体验**：SwiftUI 深度整合 macOS（通知、Spotlight、Share Extension）
5. **完整产品**：Swama 只是 API 服务端，Klee 是面向终端用户的完整 Chat App

### 时间窗口

MLX + MCP 的原生 SwiftUI 组合，市场空白期约 **6-12 个月**：
- LM Studio 随时可能推出原生 macOS 版（资金充足）
- Swama 随时可能补充 Chat UI + MCP（技术底座已具备）
- Phase 2 完成是建立先发优势的关键节点
...
- [ ] 多模态支持（VLM，图片输入）
- [ ] 对话历史持久化迁移至 SwiftData（含用户登录，邮箱注册/登录，用于跨设备同步）
- [ ] 垂直场景定制（开发者 / 内容创作者模式）
- [ ] **菜单栏模式**（参考 Swama / Ollama macOS App）：
  - 可选"菜单栏常驻"模式，无 Dock 图标，随时唤出对话窗口
  - 全局快捷键唤起（类 Raycast / Alfred 体验）
  - 参考 Swama 的 `setActivationPolicy(.accessory)` 实现，以及菜单栏状态管理
  - Swama 仓库：https://github.com/Trans-N-ai/swama
