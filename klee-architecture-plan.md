...
#### 方案 D：依赖升级 + Gated DeltaNet 修复（P0，立即）
...
**配套**：推荐模型列表增加纯文本变体（Qwen3.5-*-Text-*）。

**预期收益**：纯文本场景恢复到接近 Qwen3-8B 的速度，节省 ~1GB 内存。

#### 方案 B：VLM Adapter 模式（P1，2-3 周）
...
**实现路径**：需 fork mlx-swift-lm，在 Qwen35.swift 中将 `visionModel` 改为 optional / lazy init。
...
| 风险 | 影响 | 应对措施 |
|------|------|---------|
| Qwen3.5 MLX Swift 性能差距 | 推理不可用 | 双模型通道 + VLM Adapter + 紧跟上游 PR |
